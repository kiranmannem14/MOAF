function model = LTE_trafficmodel(trafficmodel_params,UE,HARQ_delay,max_TTI,varargin)
...
if trafficmodel_params.usetraffic_model % if the traffic models shall be used
    ...
    switch set(index)   % randomly pack one of the traffic models according to the aPrioriPdf
        ...
        case 8
%             model = traffic_models.car(UE,HARQ_delay,max_TTI);
            if trafficmodel_params.seed_traffic
                model = traffic_models.fixed_rate_tm(trafficmodel_params.packet_size,trafficmodel_params.packet_periode,max_TTI,varargin{2},UE,trafficmodel_params.traffic_stream);
            else
                model = traffic_models.fixed_rate_tm(trafficmodel_params.packet_size,trafficmodel_params.packet_periode,max_TTI,varargin{2},UE);
            end
    end
else
    model = traffic_models.fullbuffer(UE,HARQ_delay);
