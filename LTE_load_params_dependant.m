...
if isempty(strfind([v.Name],'Parallel Computing Toolbox'))
    ...
    if isfield(LTE_config,'parallel_network') && LTE_config.parallel_network
        warning('Parallel computing toolbox not available. Deactivating parallel network simulation (LTE_config.parallel_network = false)');
...
elseif LTE_config.parallel_network
    ...
    if ~isempty(t)        
        LTE_config.RandStreamSeed = ceil(t.ID + rand(1)*10^4 + cputime);
    else
        LTE_config.RandStreamSeed = ceil(rand(1)*10^4 + cputime);
    end    
    if ~LTE_config.parallel_toolbox_installed
...
if LTE_config.RRHs_enabled && ~LTE_config.runtime_precoding
    error('To use Remote Radio Heads (RRHs), run-time precoding must be employed');
end
...
% Check whether parameters of clustered UE distribution are defined properly
if strcmp(LTE_config.UE_distribution, 'clustered')
    if length(LTE_config.UE_per_eNodeB)<2
       error('Clustered UE distribution requires 2 elements in LTE_config.UE_per_eNodeB ([Total_number_of_users Number_of_UE_clusters])');
    end
    if ~isfield(LTE_config,'UE_cluster_radius')
       error('Clustered UE distribution requires to define radius of cluster');
    end
end
...
if ~isfield(LTE_config,'antenna_azimuth_offsett')
    LTE_config.antenna_azimuth_offsett = 60; % Mimic old behavior
end

if ~isfield(LTE_config,'UE')
    LTE_config.UE = [];
end
...
switch LTE_config.network_source
    case 'generated'
        switch LTE_config.network_geometry
            case {'regular_hexagonal_grid', 'GM_test_grid'}
...
switch LTE_config.shadow_fading_type
    case {'claussen','claussen_3D_UMa','claussen_3D_UMi'}
        ...
        if ~isfield(LTE_config,'claussen_seed') || ~LTE_config.claussen_seed 
            LTE_config.claussen_seed = false;
            LTE_config.claussen_stream = [];
        end
    case {'claussen_3D_UMa','claussen_3D_UMi'}
        if ~isfield(LTE_config,'deactivate_claussen_spatial_correlation')
            LTE_config.deactivate_claussen_spatial_correlation = false;
        end
        ...
        if ~isfield(LTE_config,'shadow_fading_map_resolution') ||...
                ~isfield(LTE_config,'shadow_fading_n_neighbors') ||...
                ~isfield(LTE_config,'shadow_fading_mean') ||...
                ~isfield(LTE_config,'shadow_fading_sd_LOS') ||...
                ~isfield(LTE_config,'shadow_fading_sd_NLOS') ||...
                ~isfield(LTE_config,'r_eNodeBs')
                error('When specifying a "claussen_3D_UMa or claussen_3D_UMa" shadow fading map, the following parameters need to be specified:\n  %s\n  %s\n  %s\n  %s\n  %s\n  %s\n',...
                'LTE_config.shadow_fading_map_resolution',...
                'LTE_config.shadow_fading_n_neighbors',...
                'LTE_config.shadow_fading_mean',...
                'LTE_config.shadow_fading_sd_LOS',...
                'LTE_config.shadow_fading_sd_NLOS',...
                'LTE_config.r_eNodeBs');
        end
...
switch LTE_config.network_source
    case {'capesso','trace'}
        % Do nothing
    otherwise
        if strcmp(LTE_config.antenna.antenna_gain_pattern, 'kathreinTSAntenna') || strcmp(LTE_config.antenna.antenna_gain_pattern,'TR36.873 3D antenna')
...
%% Beamforming
if ~isfield(LTE_config,'support_beamforming')
    LTE_config.support_beamforming = false; 
end
...
if ~isfield(LTE_config.scheduler_params,'fixed_CQI')
    LTE_config.scheduler_params.fixed_CQI = false;
end

if ~isfield(LTE_config,'support_handover')
    LTE_config.support_handover = true; 
end

%% Codebook selcetion for TxMode 9

if ~isfield(LTE_config,'selected_codebook')
