...
%             3D antenna
            ...
            switch class(an_antenna)
                case 'antennas.TS36942_3DAntenna'
                    ...
                    utils.miscUtils.polar2(hor_degrees/180*pi, hor_gain/max_gain, 'blue');
                    hold(the_axes,'all');
                    utils.miscUtils.polar2(ver_degrees/180*pi, ver_gain-max_gain, 'red');
                    title(the_axes,title_string);
                    hold(the_axes,'off');
                case 'antennas.TR36873_3DAntenna'
                    [hor_degrees, hor_gain, ver_degrees, ver_gain, max_gain] = ...
                        an_antenna.gain_patterns;

                    subplot(1,2,1); plot(hor_degrees,hor_gain-max_gain,'b'); xlabel('Phi [°]'); ylabel('Antenna element gain [dBi]'); title('Horizontal antenna element pattern'); ylim([-35 2]);
                    subplot(1,2,2); plot(ver_degrees,ver_gain-max_gain,'r'); xlabel('Theta [°]'); ylabel('Antenna element gain [dBi]'); title('Vertical antenna element pattern'); ylim([-35 2]);  
...
if LTE_config.show_network>0 && LTE_config.macroscopic_pathloss_is_model
    ...
    if ~isempty(all_pathloss_model_names)
        ...
        for i_=1:length(m)
            ...
            if strcmp(LTE_config.macroscopic_pathloss_model,'TR 36.873')
                pathlosses = 10*log10(networkPathlossMap.pathloss(100,:,1));
                % macroscopic_pathloss_model.pathloss(distances,LTE_config,networkPathlossMap);
                plot(pathlosses);
                title(['Macroscopic pathloss of eNodeB 1, sector 1, using ' macroscopic_pathloss_model.name ' model',],'FontSize', 10);
                xlabel('Distance [m]','FontSize', 16);
                ylabel('Pathloss [dB]','FontSize', 16);
                box on;
                grid on;
                ...
                title(['Macroscopic pathloss, using ' macroscopic_pathloss_model.name ' model',],'FontSize', 12);
                xlabel('Distance [m]','FontSize', 16);
                ylabel('Pathloss [dB]','FontSize', 16);
                box on;
                grid on;
