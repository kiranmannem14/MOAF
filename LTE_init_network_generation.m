function [sites, eNodeBs, networkPathlossMap, networkShadowFadingMap LTE_config] = LTE_init_network_generation(LTE_config,varargin)
...
%                 fprintf('Using pathloss data input as variable\n');
        ...
        if LTE_config.add_femtocells
            [sites, eNodeBs, LTE_config] = network_generation.add_femtocells(LTE_config, sites, eNodeBs, networkPathlossMap);
            ...
            switch LTE_config.shadow_fading_type
                case 'claussen'
                    ...
                    networkShadowFadingMap = channel_gain_wrappers.shadowFadingMapClaussen(...
                        ...
                        LTE_config.parallel_toolbox_installed);
                ...
                case 'claussen_3D_UMa'
                    ...
                    networkShadowFadingMap_NLOS =  channel_gain_wrappers.shadowFadingMapClaussen(...
                       ...
                       networkShadowFadingMap_OTOI =  channel_gain_wrappers.shadowFadingMapClaussen(...
                        ...
                        nShadowFadingMaps,...
                        LTE_config.shadow_fading_mean,...
                        LTE_config.shadow_fading_sd_OTOI,...
                        LTE_config.r_eNodeBs,...
                        LTE_config.claussen_stream,...
                        LTE_config.deactivate_claussen_spatial_correlation,...
                        LTE_config.parallel_toolbox_installed);
                ...
                case 'claussen_3D_UMi'
                    [LTE_config.roi_x,LTE_config.roi_y] = networkPathlossMap.valid_range;
                    if LTE_config.debug_level>=1
                        fprintf('Generating Claussen space-correlated shadow fading map for 3D UMi (combining O-to-I, LOS and NLOS maps), ');
                    end
...
%             axis equal
        ...
        if strcmp(LTE_config.channel_model.type,'3D_UMa_fading') || strcmp(LTE_config.channel_model.type,'3D_UMi_fading')
           % In case of using 3GPP 3D small scale fading model, remove antenna gain a-posteriori (after calculating sector assignment, based on max SINR)
           fprintf('Using 3GPP 3D small scale fading model. Removing antenna gain a-posteriori...\n');
           networkPathlossMap.pathloss = networkPathlossMap.pathloss.*10.^(networkPathlossMap.sector_antenna_gain/10);   
        end
