function [ UEs, extra_info, LTE_config] = LTE_init_generate_users_and_add_schedulers(LTE_config,sites,eNodeBs,networkPathlossMap,CQI_mapper,BLER_curves,networkClock)
...
switch LTE_config.UE_distribution
    ...
    case 'constant UEs per cell_TR36873'
        UE_spatial_distribution = spatial_distributions.constantElementsPerCellSpatialDistribution_TR36873(networkPathlossMap, LTE_config.UE_per_eNodeB);
    case 'traffic map'
    ...
    case '3D_positioning'
        UE_spatial_distribution = spatial_distributions.spacemodelSpatialDistribution(networkPathlossMap,LTE_config.nUEs_perCell_inGroundfloor,LTE_config.nUEs_vertical,LTE_config.height_vector);
    case 'predefined'
...
if strcmp(LTE_config.UE_distribution,'GM_test') && LTE_config.seed_position
    UE_positions = UE_spatial_distribution.generate_positions(LTE_config.position_stream);
else
...
if (~use_UE_cache) || (use_UE_cache&&~cache_file_exists)
    for u_ = 1:size(UE_positions,1)
        ...
        switch LTE_config.UE_distribution
            ...
            otherwise
                ...
                if isfield(LTE_config.UE,'walk') && strcmp(LTE_config.UE.walk,'SLvsLL')
                    ...
                    if isfield(LTE_config,'user_direction_seed') && LTE_config.user_direction_seed
                        direction = rand(LTE_config.direction_stream,1,1)*359;
        ...
        if isfield(LTE_config,'track_UE_interference')
            if LTE_config.track_UE_interference == true
                UEs(u_).track_interference = true;
            end
        end
...
for u_=1:length(UEs)
    ...
    UEs(u_).codebook = phy_modeling.miscUtils.get_all_precoding_combinations(LTE_config.selected_codebook);
    ...
    for u_ = 1:length(UEs)   
        ...
        if LTE_config.add_femtocells
            ...
            if  strcmp(UEs(u_).attached_site.site_type,'macro')
                UEs(u_).wall_loss        = LTE_config.femtocells_config.macroscopic_pathloss_model_settings.wall_loss;
    ...
    for u_ = 1:length(UEs)
        ...
        if isfield(LTE_config.traffic_models,'type') && strcmp(LTE_config.traffic_models.type,'MLaner')
            if ~LTE_config.traffic_models.traffic_patterns || (UEs(u_).attached_site.id == 5 && ~sector_UE(UEs(u_).attached_sector)) % fullbuffer UE from center eNodeB
                UEs(u_).traffic_model = LTE_trafficmodel(LTE_config.traffic_models,UEs(u_),max(LTE_config.feedback_channel_delay,0),LTE_config.simulation_time_tti,7,0);
                sector_UE(UEs(u_).attached_sector) = true;
            else
                UEs(u_).traffic_model = LTE_trafficmodel(LTE_config.traffic_models,UEs(u_),max(LTE_config.feedback_channel_delay,0),LTE_config.simulation_time_tti,7,1,LTE_config.simulation_time_tti);
            end
        else if ~isfield(LTE_config.traffic_models,'type')               
                UEs(u_).traffic_model = LTE_trafficmodel(LTE_config.traffic_models,UEs(u_),max(LTE_config.feedback_channel_delay,0),LTE_config.simulation_time_tti,6);               
            else
                UEs(u_).traffic_model = LTE_trafficmodel(LTE_config.traffic_models,UEs(u_),max(LTE_config.feedback_channel_delay,0),LTE_config.simulation_time_tti);
            end
        end
    end
