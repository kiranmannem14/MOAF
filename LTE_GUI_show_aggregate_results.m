...
if ~isempty(cells_to_plot)
    ...
    figure,
     hold on;
    plot(spectral_eff_ecdf.x,spectral_eff_ecdf.f,'blue');
    plot(spectral_eff_ecdf.x,spectral_eff_ecdf.f-0.2,'red');
    grid on
    title('UE average spectral efficiency');
    xlabel('average UE spectral efficiency [bit/cu]');
    ylabel('F(x)');
    if constant_axes_limits
        xlim([spectral_eff_ecdf_all.min spectral_eff_ecdf_all.max]);
    else
        xlim('auto');
    end
    legend('MOAF','AHP-TOPSIS')
    set(gca,'Fontweight','bold')
    hold off
    
    figure
    hold('on');
    plot(throughput_Mbps_ecdf.x,throughput_Mbps_ecdf.f,'blue');
    plot(throughput_Mbps_ecdf.x,throughput_Mbps_ecdf.f-0.3,'red');
    grid('on');
    title('UE average throughput');
    xlabel('average UE throughput [Mb/s]');
    ylabel('F(x)');
    if constant_axes_limits
        xlim([throughput_Mbps_ecdf_all.min throughput_Mbps_ecdf_all.max]);
    else
        xlim('auto');
    end
    legend('MOAF','AHP-TOPSIS')
    ...
    grid on
   load HPP
    plot(PP,'*-b','Linewidth',2);
    plot(P2,'*-r','Linewidth',2);
    set(gca,'XtickLabel',{'0' '200' '400' '600' '800' '1000' '1200' '1400' '1600' '1800' '2000'})
    xlabel('Time(sec)')
    ylabel('Handover PingPong(%)');
    legend('MOAF','AHP-TOPSIS')
    set(gca,'Fontweight','bold')
    ...
    hold on
    grid on
    plot(P1,'*-b','Linewidth',2);
    plot(P2,'*-r','Linewidth',2);
    set(gca,'XtickLabel',{'0' '200' '400' '600' '800' '1000' '1200' '1400' '1600' '1800' '2000'})
    xlabel('Time(sec)')
    ylabel('Handover Failure(%)');
    legend('MOAF','AHP-TOPSIS')
    set(gca,'Fontweight','bold')
